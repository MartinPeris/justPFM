...
fixtures generated by Netpbm, covering both byte orders and nonunit scales.
These checks have no external tool dependency and never skip for missing
Netpbm. Fixture provenance, generation commands, licensing, and the observed
scale-convention difference are recorded in
[the fixture README](tests/fixtures/netpbm/README.md).

For live writer interoperability on Linux, install Netpbm (for example,
`sudo apt-get install netpbm` on Ubuntu), then run:

```bash
tox run -e interop
```

This optional local target requires `pamtopfm` and `pfmtopam` on `PATH`; missing
tools fail explicitly. It verifies fixture regeneration and checks justPFM's
output using Netpbm's reader. The CI `Netpbm interoperability` workflow installs
Netpbm on Ubuntu 24.04 and runs the target. Its success is required by the
**Quality gate**, including when that workflow is called for a release.

Use a scale magnitude of 1 when exchanging normalized image intensities with
Netpbm: Netpbm divides by a nonunit header scale while justPFM multiplies by it.
The nonunit-scale checks expose and document that difference rather than
assuming equivalent scale semantics.
